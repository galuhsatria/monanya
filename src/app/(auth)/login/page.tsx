      ...
      <div className="w-full p-4">
        <div className="max-w-96 mx-auto w-full">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mt-4 flex flex-col gap-4 font-medium">
            ...
            <Button type="submit" disabled={isSubmitting} className="bg-accent rounded-full relative hover:bg-[#27945f] " size={'lg'}>
              {isSubmitting ? <LoaderCircle size={18} className="animate-spin" /> : 'Login'}
              {lastMethod === 'email' && <Badge className="absolute border border-accent -top-2 -right-2 z-10 rounded-full bg-white text-accent">Terakhir</Badge>}
            </Button>
          </form>
          ...
          <div className="flex gap-2 mb-3 mt-4 max-w-96 relative">
            <Button variant={'outline'} onClick={() => signIn('google')} className="relative flex-1">
              {lastMethod === 'google' && <Badge className="absolute -top-2 -right-2 z-10 rounded-full bg-accent">Terakhir</Badge>}
            ...
            <Button variant={'outline'} onClick={() => signIn('github')} className="relative flex-1">
              {lastMethod === 'github' && <Badge className="absolute -top-2 -right-2 z-10">Terakhir used</Badge>}
