...
export default function Page() {
  ...
  const onSubmit: SubmitHandler<UserLogin> = async (user) => {
    const { data, error } = await authClient.signUp.email({
      ...
      name: user.name,
      callbackURL: process.env.NEXT_PUBLIC_BASE_URL + '/email-verification',
    });
  ...
  return (
    <div className="flex items-center justify-center w-full h-screen">
      <div className="w-full p-4">
        <div className="max-w-[25rem] mx-auto w-full">
          ...
          <div className="text-left mt-4">
            ...
            <p className="font-medium text-muted-foreground text-sm">Masukan email dan password anda untuk daftar akun</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mt-4 flex flex-col gap-4 font-medium">
            <div className="flex flex-col gap-2">
              ...
              <Input type="text" {...register('name', { required: true })} placeholder="Masukan nama anda" />
            </div>
            <div className="flex flex-col gap-2">
              <label htmlFor="email" className="text-sm">
                Email
              </label>
              <Input type="email" {...register('email', { required: true })} placeholder="Tuliskan alamat email anda" />
            </div>
            <div className="flex flex-col gap-2">
              <label htmlFor="password">Password</label>

              <div className="flex gap-1 border border-border rounded-md">
                <Input type={showPassword ? 'text' : 'password'} className="border-none rounded-e-none" {...register('password', { required: true })} placeholder="Buat password anda" />
            ...
            <Button type="submit" disabled={isSubmitting} className="bg-accent rounded-full  hover:bg-[#27945f] " size={'lg'}>
              {isSubmitting ? <LoaderCircle size={18} className="animate-spin" /> : 'Lanjutkan'}
            </Button>
          </form>
          <div className="flex items-center gap-3 mt-4 text-sm">
            <hr className="inline-block flex-4 border border-border" />
            <span className="uppercase">Atau daftar dengan</span>
            <hr className="inline-block flex-4 border border-border" />
          </div>
